...
const COMMAND_DEFINITIONS = Object.freeze([
  { id: "mode1", label: "Mode 1", shortLabel: "1", expectation: "Frame A mode/profile becomes 1 / 25 / 20", kind: "mode", value: 1 },
  { id: "mode2", label: "Mode 2", shortLabel: "2", expectation: "Frame A mode/profile becomes 2 / 30 / 40", kind: "mode", value: 2 },
  { id: "mode3", label: "Mode 3", shortLabel: "3", expectation: "Frame A mode/profile becomes 3 / 40 / 99", kind: "mode", value: 3 },
  { id: "lightsOn", label: "Lights ON", shortLabel: "ON", expectation: "Frame A offset 17 bit 0 becomes 1", kind: "lights", value: true },
  { id: "lightsOff", label: "Lights OFF", shortLabel: "OFF", expectation: "Frame A offset 17 bit 0 becomes 0", kind: "lights", value: false },
]);

// Experimental G4 write strategy:
// clone the latest passive 20-byte Frame A received on FFF2, then patch only
// the already-mapped mode/profile bytes or the lights bit before writing it to FFF1.
// G2 remains raw/read-only because its packet layout has not been mapped independently.
const G4_MODE_PROFILE = Object.freeze({
  1: Object.freeze({ currentLimit: 25, speedLimit: 20 }),
  2: Object.freeze({ currentLimit: 30, speedLimit: 40 }),
  3: Object.freeze({ currentLimit: 40, speedLimit: 99 }),
});

function buildPassiveReplayCommand(command) {
  if (state.model !== "G4") throw new Error("Passive-frame controls are only mapped for G4.");
  if (!(state.latestFrameABytes instanceof Uint8Array) || state.latestFrameABytes.length !== 20) {
    throw new Error("Wait for the first 20-byte G4 Frame A.");
  }

  const bytes = new Uint8Array(state.latestFrameABytes);

  if (command.kind === "mode") {
    const profile = G4_MODE_PROFILE[command.value];
    if (!profile) throw new Error("Unknown G4 mode profile.");
    bytes[0] = profile.currentLimit;
    bytes[2] = (profile.speedLimit >> 8) & 0xFF;
    bytes[3] = profile.speedLimit & 0xFF;
    bytes[5] = command.value;
  } else if (command.kind === "lights") {
    bytes[17] = command.value ? (bytes[17] | 0x01) : (bytes[17] & 0xFE);
  } else {
    throw new Error("Unsupported passive-frame control.");
  }
...
async function connectDevice(device) {
  ...
  setNotice(`${state.model} connected. Reads and logging started; FFF1 is ${state.writeCharacteristic ? "available for mode/light controls" : "unavailable"}.`, "good");
}
...
function writeIsEnabled() {
  return Boolean(state.writesEnabled);
}
...
function updateWriteUI() {
  ...
  el.writeEnableSwitch.disabled = !connected || !characteristic || state.writeBusy || state.model !== "G4";
  el.writeEnableSwitch.checked = enabled;
  el.writeSwitchText.textContent = enabled ? "Writes on" : "Writes off";

  const passiveReady = state.model === "G4" && state.latestFrameABytes instanceof Uint8Array && state.latestFrameABytes.length === 20;
  el.commandAvailabilityNote.textContent = state.model !== "G4"
    ? "G2 remains read-only: G4 passive bytes are never reused for G2."
    : passiveReady
      ? `Ready. Latest passive Frame A: ${hex(state.latestFrameABytes)}`
      : "Waiting for the first passive 20-byte G4 Frame A.";
  el.commandAvailabilityNote.className = `command-availability-note ${passiveReady ? "ready" : "pending"}`;
  ...
  document.querySelectorAll("[data-send-command]").forEach(button => {
    ...
    let preview = null;
    try { if (command && passiveReady) preview = buildPassiveReplayCommand(command); } catch { preview = null; }
    const small = button.querySelector("small");
    if (small) small.textContent = preview ? hex(preview) : (state.model === "G4" ? "waiting for Frame A" : "G2 read-only");
    button.disabled = !enabled || state.writeBusy || !characteristic || !preview;
  });
...
function setWritesEnabled(enabled) {
  ...
  if (state.recording) addMarker(`FFF1 MODE/LIGHT WRITES ${state.writesEnabled ? "ENABLED" : "DISABLED"}`);
  updateWriteUI();
  setNotice(state.writesEnabled ? "Mode and light writes enabled." : "Writes disabled.", state.writesEnabled ? "good" : "");
}
...
function renderCommandGrid() {
  ...
  const modeCommands = COMMAND_DEFINITIONS.filter(command => command.kind === "mode");
  const lightCommands = COMMAND_DEFINITIONS.filter(command => command.kind === "lights");
  const renderButton = command => `<button class="control-command ${command.kind}" type="button" data-send-command="${command.id}" disabled>
      <span>${escapeHtml(command.label)}</span>
      <small>${state.model === "G4" ? "waiting for Frame A" : "G2 read-only"}</small>
    </button>`;
  el.commandGrid.innerHTML = `
    <section class="control-group"><p class="eyebrow">Ride mode</p><div class="mode-control-row">${modeCommands.map(renderButton).join("")}</div></section>
    <section class="control-group"><p class="eyebrow">Lights</p><div class="light-control-row">${lightCommands.map(renderButton).join("")}</div></section>`;
  updateWriteUI();
}
...
function sendPreset(id) {
  ...
  try {
    const bytes = buildPassiveReplayCommand(command);
    return sendFFF1Command({
      label: `${command.label} — passive Frame A replay`,
      bytes,
      command,
    });
  } catch (error) {
...
function exportJson() {
  ...
  const payload = {
    ...
    control_strategy: state.model === "G4" ? "clone_latest_passive_frame_a_and_patch_target_bytes" : "read_only",
    latest_passive_frame_a: state.latestFrameABytes ? hex(state.latestFrameABytes) : null,
...
function exportReport() {
  ...
  const payload = {
    ...
    write_lab: {
      ...
      control_strategy: "clone_latest_passive_frame_a_and_patch_target_bytes",
      latest_passive_frame_a: state.latestFrameABytes ? hex(state.latestFrameABytes) : null,
      write_history: state.writeHistory,
    },
