...
function updateModelUI() {
  ...
  el.decoderLabel.textContent = g4 ? "G4 FFF2 shared map v3" : "G2 FFF2 shared map v1";
...
function makeRecord({ source, bytes = null, marker = "", decoded = null, kind = "", direction = "RX", operation = "", writeLabel = "", expectedReadback = "", verificationStatus = "" }) {
  ...
  const record = {
    ...
    decoder_profile: state.model === "G4" ? "G4_FFF2_SHARED_V3" : "G2_FFF2_SHARED_V1",
...
function updateDashboard() {
  ...
  if (el.profileExpectedCurrent) el.profileExpectedCurrent.textContent = state.model === "G2" ? "Observed 15 / 20 / 25" : "Observed 25 / 30 / 40";
  if (el.profileExpectedSpeed) el.profileExpectedSpeed.textContent = state.model === "G2" ? "Observed 17 / 22 / 27" : "Observed 20 / 40 / 99";
  if (el.profileExpectedVoltage) el.profileExpectedVoltage.textContent = state.model === "G2" ? "Observed 48 V" : "Observed 60 V";
...
async function connectDevice(device) {
  ...
  addMarker(`DECODER PROFILE — ${state.model === "G4" ? "G4_FFF2_SHARED_V3" : "G2_FFF2_SHARED_V1"}`);
  ...
  setNotice(`${state.model} connected. Reads and logging started; FFF1 is ${state.writeCharacteristic ? "ready behind the write toggle" : "unavailable"}.`, "good");
}
...
function renderOffline(result, name) {
  ...
  if (!result.stats) {
    el.offlineSummary.innerHTML = `<p>This CSV mixes G2 and G4 labels. Split the capture before decoding.</p>`;
    return;
  }
  ...
  el.offlineSummary.innerHTML = `
    ...
    <div><span>Decoder</span><strong>${result.model === "G4" ? "G4 shared v3" : "G2 shared v1"}</strong></div>
