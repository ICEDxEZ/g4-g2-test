...
async function assertBluetooth() {
  if (!window.isSecureContext) throw new Error("Bluetooth requires HTTPS (or localhost). Open the deployed HTTPS site in Chrome/Edge or Safari with Beacio.");
  if (!navigator.bluetooth) {
    if (isIOSSafari()) throw new Error("Web Bluetooth is unavailable. Enable the Beacio Safari extension and reload this tab.");
    if (isAndroid()) throw new Error("Web Bluetooth is unavailable in this browser. Open the site in Google Chrome or Microsoft Edge, not an in-app browser/WebView.");
    throw new Error("Web Bluetooth is unavailable. Use Chrome/Edge, or iPhone Safari with Beacio enabled.");
  }
  if (typeof navigator.bluetooth.getAvailability === "function") {
    const available = await navigator.bluetooth.getAvailability().catch(() => true);
    if (available === false) throw new Error("Bluetooth is unavailable. Turn Bluetooth on and allow Chrome/Edge Nearby devices permission, then try again.");
  }
}
...
async function requestDevice() {
  ...
  // Do not filter by advertised FFF0. These scooters can expose FFF0 in GATT without
  // putting it in the advertisement, which makes Chrome's service-filtered picker hide
  // a perfectly valid scooter. Show nearby BLE devices, then verify FFF0 after selection.
...
async function connectDevice(device) {
  ...
  try {
    ...
    state.server = null;
    throw new Error(`The selected device does not expose the KuKirin FFF0 service. Pick the scooter from the Bluetooth list. (${normalizeError(error)})`);
  }
